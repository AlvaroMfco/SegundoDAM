...
public class AgenciaViajes implements Runnable {
	...
	public synchronized void gestionaAsientosAvion(int numAsientosReserva) {
		...
		if (aa.getAsientosLibres() >= numAsientosReserva) { 
			System.out.println(
					Thread.currentThread().getName() + " hará una reserva de " + numAsientosReserva + " plazas");
			try {
			...
			System.out.println(Thread.currentThread().getName() + " Reserva realizada con éxito."
					+ " Las plazas libres ahora son " + aa.getAsientosLibres());
		} else {
			System.out.println(Thread.currentThread().getName()+" hará una reserva de "+ numAsientosReserva +" plazas"); 
					   System.out.println("No hay plazas suficientes para " + 
